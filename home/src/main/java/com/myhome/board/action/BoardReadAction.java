...
public class BoardReadAction implements Action{
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String attName = "brd_" + boardNum; // 새로고침시 조회 수 증가를 막기 위해 세션에 정보 저장
		HttpSession session = request.getSession();
		
		if(session.getAttribute(attName) == null) { // 현재 세션이 이 글을 본적이 없다면
			dao.updateHit(boardNum); // 조회수 1 증가 
			session.setAttribute(attName,  "Y"); // attName(ex. "brd_22")을 session의 attribute로 등록 
		}
		dto = dao.select(boardNum); // DB 조회 + 결과를 Dto에 저장

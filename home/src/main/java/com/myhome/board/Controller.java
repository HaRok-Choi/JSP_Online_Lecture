...
@WebServlet("*.do") // .do로 끝나는 모든 요청에 대해서 아래의 서블릿을 호출하겠다는 의미
public class Controller extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {

		// URI에서 사용자가 요청한 페이지 추출
		String requestURI = request.getRequestURI(); // 요청된 URI(주소) : "/home/board/BoardList.do"
		int lstIndex = requestURI.lastIndexOf("/") + 1; // 13 + 1 --> 14
		String requestPage = requestURI.substring(lstIndex); // "BoardList.do"

		System.out.println("requestURI : " + requestURI); // "/home/board/BoardList.do"
		...
		try {
			if (requestPage.equals("BoardList.do")) {
				action = new BoardListAction(); // Action 인터페이스를 구현하는 클래스임
			...
			if (actionForward != null) {
				if (actionForward.isRedirect()) { // 리다이렉트 방식으로 nextPath 이동
